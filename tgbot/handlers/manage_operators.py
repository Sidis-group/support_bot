...
async def see_all_operators(call: CallbackQuery, callback_data: dict):
    await call.answer()
    page = int(callback_data.get('page', 0))
    markup = await create_markup_for_view_operators(page)
...
async def create_markup_for_view_operators(page: int):
    operators: list[schemas.Operator] = await db.get_operators()
    markup = inline.page_navigation_for_operators_markup(operators, page)
    return markup

async def view_info_about_operator(call: CallbackQuery, callback_data: dict):
    await call.answer()
    ...
    await call.message.edit_text(
        ...
        reply_markup=inline.delete_operator_markup(operator.telegram_id)
    )
